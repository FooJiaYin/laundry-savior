        ...
        : state.currentMachine != null && state.currentMachine!.status.code == StatusCode.available
            ...
            : state.status == Status.idle
                ? _statusCard(
                    ...
                    onTap: () {
                      state.update(status: Status.waitingFloor);
                    ...
                    : state.status == Status.waitingAll
                        ...
                        : state.status == Status.using
                            ? state.currentMachine?.status.code == StatusCode.in_use
                                ? _statusCard(
                                    title: "${state.waitingMachine == WashingMachine ? 'Washing' : 'Drying'} in progress",
                                    ...
                                    actionWidget: ActionText('${state.currentMachine?.status.durationEstimated!.inMinutes} min left', color: ThemeColors.royalBlue, icon: null),
                                    onTap: () => showDialog(context: context, builder: (context) => const SelectDormDialog()),
                                  )
                                : _statusCard(
